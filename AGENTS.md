...
| --- | --- |
| `linear [identifier] [--from <worktree\|branch>]` | Takes a Linear issue ID (e.g. `ENG-123`); requires `LINEAR_API_KEY`; fetches issue details, prompts to set the issue to In Progress and assign to yourself, then delegates to `create` with the resolved branch name and issue title/description passed as agent args. |
...
| `complete-linear` | Hidden command; outputs `identifier\ttitle` for the current user's active Linear issues; used by shell completions for the `linear` command. |
